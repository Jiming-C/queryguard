...
> Free tiers may log prompts and train on them, and they rate-limit hard (roughly a handful of requests per minute). That's fine here: the database is synthetic sample data, and each question is only three calls. Don't point this at real or sensitive data on a free key.
